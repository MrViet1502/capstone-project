...
unsigned char calculate_checksum(const unsigned char *data, int length)
{
    ...
    { // Bỏ start (0x01) và end byte (0x0A)
        checksum ^= data[i];
    }
    return checksum;
}
...
void listen_uart(XLaccess xlChanMaskTx)
{
    ...
    int bytes_received;
    uint8_t recv_buffer[BUFFER_SIZE] = ""; // Bộ đệm nhận dữ liệu
    ...
    {
        // Nhận dữ liệu
        int bytes_received = recv(sockfd, buffer, 1, 0);
        if (bytes_received != 1 || buffer[0] != 0x01)
        {
            printf("Không tìm thấy Start Byte (0x01). Bỏ qua frame.\n");
            continue;
        }

        bytes_received = recv(sockfd, buffer + 1, 1, 0);
        if (bytes_received != 1)
        {
            printf("Không nhận data length hợp lệ. Bỏ qua frame.\n");
            continue;
        }

        bytes_received = recv(sockfd, buffer + 2, buffer[1], 0);
        if (bytes_received != buffer[1])
        {
            printf("Không nhận đủ payload. Bỏ qua frame.\n");
            continue;
        }
        ...
        bytes_received = recv(sockfd, buffer + 2 + buffer[1], 1, 0);
        if (bytes_received != 1)
        {
            printf("Không nhận đủ payload. Bỏ qua frame.\n");
            continue;
        ...
        // Kiểm tra Checksum
        unsigned char received_checksum = buffer[buffer[1] + 2];
        unsigned char calculated_checksum = calculate_checksum(buffer, buffer[1] + 2);
        if (received_checksum != calculated_checksum)
        {
            printf("❌ Checksum lỗi! Nhận: 0x%02X - Mong đợi: 0x%02X\n", received_checksum, calculated_checksum);
            continue;
        }

        bytes_received = recv(sockfd, buffer + 2 + buffer[1] + 1, 1, 0);
        if (bytes_received != 1)
        {
            printf("Không nhận được End Byte (0x0A). Bỏ qua frame.\n");
            continue;
        }

        {
            int frames = (buffer[1] - 1) / 8;
            int remainingLength = buffer[1];
            int count = 0;
            while (remainingLength > 0)
            {

                // Dữ liệu hợp lệ và gửi qua busmaster
                CustomCANMessage msg;
                msg.id = 0x111 + buffer[1] + count; // ID được giả sử trong phần dữ liệu
                ...
                {
                    msg.dlc = remainingLength;
                    remainingLength = 0;
                }
                // // Chuyển dữ liệu vào msg.data
                for (int i = 0; i < msg.dlc; i++)
                {
                    msg.data[i] = buffer[2 + i + msg.dlc * count];
                }
                count++;
                // Gửi frame thông qua xldriver
                ...
                {
                    printf("Lỗi khi truyền dữ liệu: %d\n", status);
                }
